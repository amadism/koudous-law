<template>
    <UContainer class="py-6 lg:py-24 flex flex-col-reverse lg:flex-row gap-4 lg:items-center">
        <div class="lg:w-1/2">
            <img :src="data.image_url" :alt="data.title" class="max-h-[500px] lg:min-w-[400px] rounded object-center lg:object-left" />
        </div>
        <div class="lg:w-1/2">
            <h2 class="text-xl lg:text-2xl font-bold text-primary mb-5">{{ data.title }}</h2>
            <p class="text-lg mb-4">{{ data.subtitle }}</p>
            <p class="text-lg mb-4">{{ data.disc1 }}</p>
            <p class="text-lg mb-4">{{ data.disc2 }}</p>
            <UButton class="px-3 lg:px-6 py-2 text-lg" 
            :to="data.buttonPath">{{ data.buttonText }}</UButton>
        </div>
    </UContainer>
</template>

<script setup>
defineProps({
    data: {
        type: Object
    }
})
</script>